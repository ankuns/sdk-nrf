...
static void fem_temperature_compensation_thread(void *dummy1, void *dummy2, void *dummy3)
{
	...
	while (true) {
		...
		if (mpsl_fem_nrf2220_temperature_changed(fem_temperature)) {
			printk("   need update\n");
			/* As a follow-up we need to perform mpsl_fem_nrf2220_temperature_update_request
			 * This function will perform asynchronous i2c operations. We need 
			 * to have granted exclusive access before
			 */

			const struct device *i2c_bus_dev = DEVICE_DT_GET(DT_BUS(MPSL_FEM_TWI_IF));

			printk("i2c_nrfx_twim_exclusive_access_acquire\n");
			int ret = i2c_nrfx_twim_exclusive_access_acquire(i2c_bus_dev, K_FOREVER);
			...
			mpsl_fem_nrf2220_temperature_update_request(fem_temperature_update_cb);
		
			/* Let's wait until the operation is finished */
			k_sem_take(&fem_temperature_updated_sem, K_FOREVER);

			printk("i2c_nrfx_twim_exclusive_access_release\n");
