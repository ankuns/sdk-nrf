...
void nfc_tnep_tag_rx_msg_indicate(const uint8_t *rx_buffer, size_t len)
{
	...
	nfc_tnep_tag_signalling_rx_msg_event_raise(TNEP_EVENT_MSG_RX_NEW);
}
...
void nfc_tnep_tag_process(void)
{
	...
	if (nfc_tnep_tag_signalling_rx_msg_event_check_and_clear(&event)) {
		/* Run TNEP State Machine - prepare response */
		tnep_state_machine[atomic_get(&current_state)].process(event);
	}

	if (nfc_tnep_tag_signalling_tx_msg_event_check_and_clear(&event)) {
		/* Run TNEP State Machine - prepare response */
		tnep_state_machine[atomic_get(&current_state)].process(event);
	}
}
...
{
	nfc_tnep_tag_signalling_tx_msg_event_raise(TNEP_EVENT_TAG_SELECTED);
}
